...
int main(int argc, char** argv) {
  ...
  try {
    ...
    Rcpp::Rcout << app.help() << std::endl;
    return runHighsReturn(highs, 0);
  } catch (const CLI::CallForAllHelp& e) {
    Rcpp::Rcout << app.help();
    return runHighsReturn(highs, 0);
  } catch (const CLI::RequiredError& e) {
    Rcpp::Rcout << "Please specify filename in .mps|.lp|.ems format."
              << std::endl;
    return runHighsReturn(highs, HighsStatus::kError);
  } catch (const CLI::ExtrasError& e) {
    Rcpp::Rcout << e.what() << std::endl;
    Rcpp::Rcout << "Multiple files not supported." << std::endl;
    return runHighsReturn(highs, HighsStatus::kError);
  } catch (const CLI::ArgumentMismatch& e) {
    Rcpp::Rcout << e.what() << std::endl;
    Rcpp::Rcout << "Too many arguments provided. Please provide only one."
              << std::endl;
    return runHighsReturn(highs, HighsStatus::kError);
  } catch (const CLI::ParseError& e) {
    Rcpp::Rcout << e.what() << std::endl;
    // app.Rcpp::stop("Something went wrong!") should be called from main.
    return runHighsReturn(highs, app.Rcpp::stop("Something went wrong!"));
  }
